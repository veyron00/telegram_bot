...
from functions.func import change_city, search_hotels, change_photo, miles_to_km, period_of_stay, hotel_stars, \
    checking_date_entry, check_date_in, minimum_check_in_date, calc_departure_date, checking_departure_date, \
    checking_date_entry_date_departure
...
from states.states import FSMLowPrise


@dp.message_handler(commands="lowprice", state=None)
async def cmd_lowprice(message: types.Message) -> None:
    """
    Хэндлер для отлавливания команды lowprice.
    При нажатии вызове команды lowprice бот предлагает ввести название города.

    :param message: types.Message
    :return: None
    """
    await FSMLowPrise.user_request.set()
    await message.reply("Введите название города 🗺")
    logger.info(f"user_id - {message['from']['id']}, command_name - {message.text}")


@dp.message_handler(state=FSMLowPrise.user_request)
async def choose_city(message: types.Message, state: FSMContext) -> None:
    """
...
@dp.callback_query_handler(lambda query: query.data.startswith('next'), state=FSMLowPrise.city_id)
async def choose_date_in(query: types.CallbackQuery, state: FSMContext) -> None:
    """
...
@dp.message_handler(state=FSMLowPrise.check_in_date)
async def choose_date_out(message: types.Message, state: FSMContext) -> None:
    """
    ...
    if checking_date_entry(message.text):
        if check_date_in(message.text):
            ...
            await message.answer(f"Дата выезда не должна превышать 28 дней с даты заезда."
                                 f"\nКрайняя дата выезда {calc_departure_date(message.text)}")
...
@dp.message_handler(state=FSMLowPrise.check_out_date)
async def choose_hotel_num(message: types.Message, state: FSMContext) -> None:
    """
...
@dp.message_handler(state=FSMLowPrise.hotels_num)
async def need_photo(message: types.Message, state: FSMContext) -> None:
    """
...
@dp.callback_query_handler(text='yes', state=FSMLowPrise.photo)
async def choose_photo_num(callback: types.CallbackQuery, state: FSMContext):
    """
...
@dp.callback_query_handler(text='no', state=FSMLowPrise.photo)
async def no_photo(callback: types.CallbackQuery, state: FSMContext) -> None:
    """
        ...
        if len(hotels) > 0:
            ...
            for key, value in hotels.items():
                ...
                await callback.message.answer(f'{28 * "-"} Отель {nums} {27 * "-"}'
                                              ...
                                              f'\nСтоимость за ночь <i><b>{round(int(value[0]) / days_of_stay)}</b'
                                              f'></i> RUB '
...
@dp.message_handler(state=FSMLowPrise.num_photo)
async def with_photo(message: types.Message, state: FSMContext) -> None:
    """
...
def register_handler_low_price(dp: Dispatcher):
    dp.register_message_handler(cmd_lowprice, commands="lowprice", state=None)
    dp.register_message_handler(choose_city, state=FSMLowPrise.user_request)
    dp.register_callback_query_handler(choose_date_in, lambda query: query.data.startswith('next'),
                                       state=FSMLowPrise.city_id)
    dp.register_message_handler(choose_date_out, state=FSMLowPrise.check_in_date)
    dp.register_message_handler(choose_hotel_num, state=FSMLowPrise.check_out_date)
    dp.register_message_handler(need_photo, state=FSMLowPrise.hotels_num)
    dp.register_callback_query_handler(choose_photo_num, text='yes', state=FSMLowPrise.photo)
    dp.register_callback_query_handler(no_photo, text='no', state=FSMLowPrise.photo)
    dp.register_message_handler(with_photo, state=FSMLowPrise.num_photo)
